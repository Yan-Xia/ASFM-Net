...
def inputs_process(inputs, Nin, npts, batch_size):
    outputs=[]
    npts_new = np.zeros(batch_size-1,)
    npts_new[0] = npts[0]
    for i in range(1, batch_size-1):
        npts_new[i] = npts[i]+npts_new[i-1]
    npts_new = npts_new.astype(int)
    for f in np.split(inputs, npts_new, axis=1):
        f = np.squeeze(f)
        f = pad_cloudN(f, Nin)
